...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2022/02/03 15:31:19 by naqua             #+#    #+#             */
/*   Updated: 2022/02/28 10:57:10 by naqua            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_init_philo(t_arg *args)
{
	...
	while (i < args->nbr_philo)
	{
		...
		philos[i].nbr_of_meals_1 = args->nbr_of_meals;
		...
		philos[i].r_f = &args->forks[(philos[i].philo_id + 1)
			% args->nbr_philo];
		philos[i].arg = args;
		i++;
	}
...
void	ft_init_threads(t_arg *args)
{
	...
	while (nbr_ph--)
		pthread_create(&threads[nbr_ph], NULL, ft_process,
			(void*)&args->all_philos[nbr_ph]);
